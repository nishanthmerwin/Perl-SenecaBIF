...
my $old_seq = $frame1->Entry(-width => '20', -relief => 'sunken')->grid(-row=>1 , -column=>2);

# Adds an entry button and saves its state in $button 
my $button = $frame1->Button(-text => "Clean Up", -command=> \&clean_up)->grid(-row=>2 , -column=>1);
# Adds a second text field
$frame1->Label(-text => "New Sequence")->grid(-row=>3 , -column=>1);
my $new_seq = $frame1->Entry(-width => '20', -relief => 'sunken')->grid(-row=>3 , -column=>2);

# Makes sure its not editable
$new_seq->configure(-state=>'disabled');

# Makes a reset button
my $reset_button = $frame1->Button(-text=> "Reset", -command=>\&reset)->grid(-row=>2 , -column=>2);
