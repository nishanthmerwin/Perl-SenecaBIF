...
my $labelFrame = $frame1->Frame(-bg=>'black')->pack(-side=>'top', -expand=>'1', -fill=>'both');

my $orgFrame = $frame1->Frame(-bg=>'black')->pack(-side=>'top', -expand=>'1', -fill=>'both');


my $cleanFrame = $frame1->Frame(-bg=>'black')->pack(-side=>'top', -expand=>'1', -fill=>'both');



my $newLabFrame = $frame1->Frame(-bg=>'black')->pack(-side=>'top', -expand=>'1', -fill=>'both');
...
$labelFrame->Label(-text => "Original Sequence",-bg=>'black',-fg=>'white')->pack(-side => 'top');

# Retreives the information from thee entry box
my $old_seq = $orgFrame->Entry(-width => '20', -relief => 'sunken')->pack(-side => 'top');

# Adds an entry button and saves its state in $button 
my $button = $cleanFrame->Button(-text => "Clean Up", -command=> \&clean_up)->pack(-side => "top");

# Adds a second text field
$newLabFrame->Label(-text => "New Sequence")->pack(-side => 'top');
my $new_seq = $newFrame->Entry(-width => '20', -relief => 'sunken')->pack(-side => 'top');

# Makes sure its not editable
$new_seq->configure(-state=>'disabled');

# Makes a reset button
my $reset_button = $resetFrame->Button(-text=> "Reset", -command=>\&reset)->pack(-side=>'top');
